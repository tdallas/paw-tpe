...
const Reservations = ({ history }) => {
  ...
  const createQueryString = () =>
    `startDate=${startDate}&endDate=${endDate}&email=${email}&lastName=${lastName}`;
      ...
      <Container fluid="md" className={classes.container}>
        <Row className={classes.row}>
          <Col xs={12} md={6}>
            <DatePicker
              Id="from"
              label={t("reservation.date.start")}
              value={startDate}
        ...
        <InfoSimpleDialog open={showDialog} onClose={handleDialogClose}>
          {errorStatusCode === 400
            ? t("reservation.date.required")
            : t("reservation.checkin.error")}
        </InfoSimpleDialog>
        <br />
        <Row
